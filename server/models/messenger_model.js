  ...
  try {
    const [messages] = await conn.query(
      `SELECT msg.room, msg.sender_id, msg.receiver_id, msg.content, msg.create_time, 
       ...
       FROM ( 
        SELECT * FROM message 
        WHERE id in (
          SELECT MAX(id) FROM message WHERE sender_id = ? OR receiver_id = ? GROUP BY room)) AS msg 
       LEFT JOIN user AS sender_info on msg.sender_id = sender_info.id 
       LEFT JOIN user AS receiver_info on msg.receiver_id = receiver_info.id 
    ...
    const [count] = await conn.query(
      `SELECT msg_count.room, COUNT(msg_count.create_time) AS msg_num, MAX(msg_count.create_time) AS last_msg 
       FROM (
        SELECT pi.message.room, pi.message.create_time 
       FROM pi.message 
       ...
       WHERE msg_count.create_time > last_check.last_time 
       OR last_check.last_time IS NULL 
