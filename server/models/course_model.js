...
const getCourses = async (
  ...
  try {
    ...
    const condition = {
      sql: [
        "SELECT course.id, course.cover, course.title, course.price, course.upload_time, COUNT(course_student.user_id) AS students_num \
        FROM course \
    ...
    if (requirement.order == "recommend" && requirement.token !== undefined) {
      ...
      recommendSql = `WHERE course.id IN ( 
        SELECT user_favorites.favorites_id 
        FROM ( 
          SELECT course_favorites.course_id AS favorites_id 
          FROM course_favorites 
          WHERE user_id=?) AS user_favorites 
          LEFT JOIN ( 
            SELECT course_student.course_id AS bought_id 
            FROM course_student 
            WHERE user_id=?) AS user_bought 
          ON user_favorites.favorites_id=user_bought.bought_id 
          WHERE user_bought.bought_id IS NULL) 
        OR course.id IN ( 
          SELECT user_recommend.course_id 
          FROM ( 
            SELECT DISTINCT course_student.course_id 
            FROM pi.course_student 
            WHERE course_student.user_id IN ( 
              SELECT DISTINCT course_student.user_id 
              FROM pi.course_student 
              WHERE course_student.user_id != ? 
              AND course_student.course_id IN ( 
                SELECT course_student.course_id 
                FROM course_student 
                WHERE course_student.user_id=?))) AS user_recommend 
              LEFT JOIN ( 
                SELECT course_student.course_id AS bought_id 
                FROM course_student WHERE user_id=?) AS user_bought 
                ON user_recommend.course_id=user_bought.bought_id 
                WHERE user_bought.bought_id IS NULL 
              )GROUP BY course.id `;
      ...
      if (requirement.category !== "all") {
        let categorySql = `LEFT JOIN category 
        ...
        let hashtagSql = `LEFT JOIN course_tag 
...
const searchCourses = async (pageSize, paging = 0, keyword) => {
  ...
  try {
    await conn.query("START TRANSACTION");
    const courseSql = `
      SELECT course.id, course.cover, course.title, course.price, COUNT(course_student.user_id) AS students_num, user.name 
      FROM course 
      LEFT JOIN user 
    ...
    const livestreamSql = `
      SELECT livestream.id, livestream.cover, livestream.title, livestream.start_time, COUNT(livestream_student.user_id) AS students_num, user.name 
      FROM livestream 
      LEFT JOIN user 
